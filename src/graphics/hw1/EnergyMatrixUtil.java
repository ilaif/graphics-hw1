...
public final class EnergyMatrixUtil {
    ...
    static Matrix getEnergyMatrix001(BufferedImage inImg, EnergyType energyType){
    ...
    private static Matrix getGradientMatrix(BufferedImage inImg) {
        //Roy - CHECKED(NOTES: CAN PRACTICLY MISUSE OUTER LOOP BY SPECIFYING WHICH NEIGHBOURS ARE LEGAL MATHEMATICALLY,
        //SEE getGradientMatrix001 METHOD).
        //GENERAL QUESTIONS TO ILAI:
        // 1. BUG: i,j ARGUMENTS TO getDiff METHOD ARE ALWAYS (-1,0,1)X(-1,0,1) PAIRS.
        // SOL: USE x+i,y+i ARGUMENTS INSTEAD
        // 2. WHY SETTING TO (y,x) ENTRY AND NOT (x,y)
        // 3. WHY sumDiff IS INTEGER AND NOT DOUBLE?
        // 4. neighbours SHOULD START COUNTING FROM -1 TO IGNORE THE (i,j) ENTRY AS A NEIGHBOUR OF ITSELF(?)
    ...
    private static Matrix getGradientMatrix001(BufferedImage inImg){
        int width = inImg.getWidth();
        int height = inImg.getHeight();
        int neighbours;
        ...
        for(int x=0; x<width ; x++) {
            for (int y = 0; y < height; y++) {
                ...
                mGradientMatrix.set(x,y,normalizedGradient);
            }
        }
    ...
    //Roy-CHECKED(NOTES: IF USING getGradientMatrix001 THEN IF STATEMENT IS UNNECESSARY)
    //NOTES TO MYSELF[ROY] - FIGURE OUT HOW BUFFEREDIMAGE OBJECT IS, AND COLOR.
    ...
    private static Matrix getEntropyMatrix(BufferedImage inImg) {
        ...
        int width = inImg.getWidth();
        int height = inImg.getHeight();
        Matrix mEntropyMatrix = new Matrix(width,height);
    ...
    //11X11 pixel window surrounding him grayscale values sum. using getGrayScaleMatrix.
        ...
        Matrix mPmnMatrix = new Matrix(width,height);
        Matrix mGrayScaleMatrix=getGrayScaleMatrix(inImg);

        for(int x=0; x<width; x++){
            for (int y = 0; y < height; y++) {
                sumNeighboursGray=0;
                gray=mGrayScaleMatrix.get(x,y);
                for(int k=Math.max(x-4,0); k<Math.min(x+4,width); k++){
                    for(int j=Math.max(y-4,0); j<Math.min(y+4,height); j++) {
                        sumNeighboursGray += mGrayScaleMatrix.get(k, j);
                    }
                }
                pmn=gray/sumNeighboursGray;
                mPmnMatrix.set(x,y,pmn);
            }
        }
        return(mPmnMatrix);
    }
    ...
    //calculates grayscale matrix using getGrayScale function.
        int width = inImg.getWidth();
        int height = inImg.getHeight();
        Matrix mGrayScaleMatrix=new Matrix(width,height);
        double gray;

        for(int x=0; x<width; x++){
            for (int y = 0; y < height; y++) {
                gray=getGrayScale(inImg,x,y);
                mGrayScaleMatrix.set(x,y,gray);
            }
        }
